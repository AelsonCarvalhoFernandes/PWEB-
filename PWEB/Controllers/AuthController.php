    ...
    function forgotPasswordValidate() {
        ...
        if ($data['password'] == $password) {
            if ($newPassword == $confirmNewPassword) {
                ...
                $this->userRepository->closeConnection();
                header("Location: /login");
                return view('Login', ['Success' => 'Senha alterada com sucesso']);

            } else {
                return view('ForgotPassword', ['Error' => 'As senhas não coincidem']);
            }
        } else {
            return view('ForgotPassword', ['Error' => 'Senha atual ou Email incorreto']);
        }
    }
}